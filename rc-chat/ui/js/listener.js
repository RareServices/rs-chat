        ...
        if (e.key === 'Enter' || e.key == "Backspace") {
            ...
            if (command){
                if (chat_types[command]){
                    ...
                    $.post(`https://rc-chat/send`,JSON.stringify({
                        command : command,
                        msg : text,
                        type : currentChat,
                    }));
        ...
        if (event.data.action == "show"){
            ...
            switch (event.data.type){
                ...
                case 'ambulance':
                    ...
                    break;
                case 'reply':
                    sendReplyMessge(event.data.msg,event.data.sender_data);
                    break;
                case 'announce':
                    sendAnnounceMessge(event.data.msg,event.data.sender_data);
                    break;
                case 'developer':
                    sendDeveloperMessge(event.data.msg,event.data.sender_data);
                    break;
...
function sendReplyMessge(msg){
    let appendHtml = `
    <tr>
        <td><div id="chat-category" class="category-reply"><div id="category-text">REPLY</div></div></td>
        <td class="reply-gradient">
            <div id="category-icon" class="icon-reply"><i class="fas fa-info"></i></div>
            <div id="chat-text">`+msg+`</div>
        </td>
    </tr>
    `
...
function sendAnnounceMessge(msg){
    let appendHtml = `
    <tr>
        <td><div id="chat-category" class="category-announce"><div id="category-text">SERVER</div></div></td>
        <td class="announce-gradient">
            <div id="category-icon" class="icon-announce"><i class="fas fa-info"></i></div>
            <div id="chat-text">`+msg+`</div>
        </td>
    </tr>
    `
...
function sendDeveloperMessge(msg){
    let appendHtml = `
    <tr>
        <td><div id="chat-category" class="category-developer"><div id="category-text">DEV</div></div></td>
        <td class="developer-gradient">
            <div id="category-icon" class="icon-developer"><i class="fas fa-info"></i></div>
            <div id="chat-text">`+msg+`</div>
        </td>
    </tr>
    `
...
function showMessages(){
    $("#wrap-chat").show();
    $("#wrap-chat-gradient").show();
}
function hideMessages(){
    $("#wrap-chat").fadeOut(2000);
    $("#wrap-chat-gradient").fadeOut(2000);
}
