...
RegisterNetEvent('rs-chat:addCommands')
AddEventHandler('rs-chat:addCommands', function(commands)
    ...
    TriggerServerEvent('rs-chat:serverCommands')
...
RegisterNetEvent('rs-chat:recieveCommands')
AddEventHandler('rs-chat:recieveCommands', function(commands)
...
Citizen.CreateThread(function()
    ...
    TriggerEvent('rs-chat:addCommands')
    if loaded then
        while true do
            Citizen.Wait(0)
            if IsControlPressed(0, isRDR and `INPUT_MP_TEXT_CHAT_ALL` or 245) then
                ...
                opened = true
                local chatType = {}
                ESX.TriggerServerCallback('rs-chat:getGroup', function(group)
...
RegisterNetEvent('rs-chat:addmessage')
AddEventHandler('rs-chat:addmessage', function(data)
    SendNUIMessage({
        action = 'msg',
        type = data.type,
        msg = data.msg,
...
RegisterNUICallback("send", function(data)
    ...
    if data.msg:sub(1, 1) == '/' then
        TriggerServerEvent('rs-chat:logs', data ,Config.Webhooks.commands)
        ExecuteCommand(data.msg:sub(2))
    else
        TriggerServerEvent('rs-chat:sendmsg', data)
    end
...
RegisterNetEvent('rs-chat:setPlayerIdentifiers')
AddEventHandler('rs-chat:setPlayerIdentifiers', function(identifiers)
    playerIdentifiers = identifiers
end)
Citizen.CreateThread(function()
    while playerIdentifiers == {} or playerIdentifiers == nil do
        Citizen.Wait(1000)
        TriggerServerEvent('rs-chat:getPlayerIdentifiers')
    end
...
function chat(i)
    TriggerServerEvent('rs-chat:announce', Config.prefix .. Config.messages[i])
end
RegisterCommand(Config.CommandOnOffAutoChat, function()
    ...
    TriggerServerEvent('rs-chat:announce', '[pAnnounce] automessages are now ' .. status)
